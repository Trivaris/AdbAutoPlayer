<script lang="ts">
    ...
    let activeGameMenuButtons: MenuButton[] = $derived.by(() => {
        if (activeGame?.MenuOptions && activeGame.MenuOptions.length > 0) {
            const menuButtons: MenuButton[] = activeGame.MenuOptions.map((menuOption) => ({
                label: menuOption.Label,
                callback: (event) => startGameProcess(event, activeGame, menuOption.Args),
                active: false,
                alwaysEnabled: false,
            }));
    ...
    function startGameProcess(event: PointerEvent, game: games.Game|null, args: string[]) {
        if (game === null) {
            return;
        }
    ...
    function onMainConfigSave(configObject: object) {
        SaveMainConfig(
            config.MainConfig.createFrom(configObject)
        ).catch((err) => {
            alert(err)
        }).finally(() => {
    ...
    function onAFKJourneyConfigSave(configObject: object) {
        ...
        SaveAFKJourneyConfig(
            activeGame,
            afkjourney.Config.createFrom(configObject)
        ).catch((err) => {
            alert(err)
        }).finally(() => {
            showConfigForm = false;
